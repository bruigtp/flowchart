...
safo |> 
  as_fc(label = "Patients assessed for eligibility") |>
  fc_filter(!is.na(group), label = expression(paste("Mixed: ", italic("Italics"), " and ", bold("bold"))), show_exc = TRUE) |> 
  fc_split(group) |> 
  fc_draw()
```
...
safo |> 
  as_fc(label = "Patients assessed for eligibility") |>
  fc_filter(!is.na(group), label = expression(paste("y = ", beta[0], " + ", beta[1], X[1], " + ", beta[2], X[2]^2)), show_exc = TRUE) |> 
  fc_split(group) |> 
  fc_draw()
```
