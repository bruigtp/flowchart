...
- `as_fc()` initializes a dataframe into a `fc` object that can be manipulated by the package.
...
The first step is to initialise the flowchart with `as_fc`. The last step, if we want to visualise the created flowchart, is to draw the flowchart with `fc_draw`. In between we can combine the functions `fc_split`., `fc_filter`, `fc_merge`, `fc_stack` with the operator pipe (`%>%`) to create very complex flowchart structures.
...
We can filter the flowchart using `fc_filter()` specifying the logic in which the filter is to be applied:
...
We can combine different flowcharts horizontally using `fc_merge()`:
...
We can combine different flowcharts vertically using `fc_stack()`:
...
<tbody>
  ...
  <tr>
   ...
   <td style="text-align:left;"> modify the color of the box border. </td>
  </tr>
</tbody>
</table>
...
<tbody>
  <tr>
   ...
   <td style="text-align:left;"> to hide the first initial box created by this function. </td>
  </tr>
</tbody>
</table>
...
<tbody>
  ...
  <tr>
   ...
   <td style="text-align:left;"> modify the color of the exclusion box border. </td>
  </tr>
</tbody>
</table>
...
<tbody>
  ...
  <tr>
   ...
   <td style="text-align:left;"> unit specifying the lenth of the arrow head. </td>
  </tr>
  <tr>
   <td style="text-align:left;"> `arrow_ends=` </td>
   <td style="text-align:left;"> specify the ends of the line to draw arrow head (last/first/both). </td>
  </tr>
  <tr>
...
For example, we could fully customise the text in the exclusion box, without following a predefined text pattern:
...
Second, let's create and customise the flowchart using the features in the package:
